...
class PreviewPanel(QWidget):
    ...
    preview_clicked = pyqtSignal(int, int, Qt.KeyboardModifier)
    ...
    def mousePressEvent(self, event) -> None:  # noqa: N802
        ...
        panel_pos = event.position()
        label_rect = self._image_label.geometry()
        if not label_rect.contains(int(panel_pos.x()), int(panel_pos.y())):
            return super().mousePressEvent(event)

        local_x = panel_pos.x() - label_rect.x()
        local_y = panel_pos.y() - label_rect.y()

        shown_w = shown.width()
        shown_h = shown.height()
        x0 = (self._image_label.width() - shown_w) / 2.0
        y0 = (self._image_label.height() - shown_h) / 2.0

        if not (x0 <= local_x < x0 + shown_w and y0 <= local_y < y0 + shown_h):
            return super().mousePressEvent(event)

        img_h, img_w = self._image_size
        img_x = int((local_x - x0) * img_w / shown_w)
        img_y = int((local_y - y0) * img_h / shown_h)

        img_x = max(0, min(img_w - 1, img_x))
        img_y = max(0, min(img_h - 1, img_y))
        self.preview_clicked.emit(img_x, img_y, event.modifiers())
        event.accept()
